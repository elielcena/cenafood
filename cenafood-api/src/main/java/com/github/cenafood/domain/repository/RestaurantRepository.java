...
public interface RestaurantRepository extends JpaRepository<Restaurant, Long> {
    ...
    static final String JPQL_RESTAURANT =
            "FROM Restaurant r LEFT JOIN FETCH r.kitchen k LEFT JOIN FETCH r.address.city c LEFT JOIN FETCH c.state LEFT JOIN FETCH r.users u";
    ...
    @Query("SELECT CASE WHEN count(1) > 0 then true else false end FROM Restaurant r JOIN r.users u WHERE r.id = :idRestaurant AND u.id =:idUser")
    Boolean existsUsers(Long idRestaurant, Long idUser);
